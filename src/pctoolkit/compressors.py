...
from .download import DOWNLOAD_DIR, download_model
from typing import List
from pathlib import Path

base_dir = Path(__file__).resolve().parent
model_dir = Path(DOWNLOAD_DIR) / "pretrained_models/gigaword-L8/"
if not model_dir.exists():
    download_model("gigaword-L8")


class PromptCompressor:
    def __init__(
        self,
        type: str = "SCCompressor",
        lang: str = "en",
        model="gpt2",
        device="cuda",
        ...
        token: str = "",
        tokenizer_dir: str = str(base_dir / "sentence-transformers/paraphrase-distilroberta-base-v2"),
    ):
        self.type = type
        if self.type == "SCCompressor":
            ...
            if model_dir:
                self.compressor = SCRLCompressor(
                    model_dir=model_dir, device=device, tokenizer_dir=tokenizer_dir
                )
            else:
    ...
    def compressgo(
        ...
        if self.type == "SCCompressor":
            return self.compressor.compress(
                original_prompt=original_prompt, ratio=ratio, level=level
            )
        elif self.type == "KiSCompressor":
            return self.compressor.compress(
            ...
            return self.compressor.compress(
                original_prompt=original_prompt, ratio=ratio
            )
