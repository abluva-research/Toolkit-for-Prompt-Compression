...
class LexicalUnits:
    ...
    def add_to_head(self, token, self_info):
        return LexicalUnits(
            self.unit_type, [token] + self.text, [self_info] + self.self_info
        )

    def add_to_tail(self, token, self_info):
        return LexicalUnits(
            self.unit_type, self.text + [token], self.self_info + [self_info]
        )
...
class SelectiveContext:
    ...
    def _prepare_model(self):
        ...
        if self.model_type == "gpt2":
            ...
            self.tokenizer = LlamaTokenizer.from_pretrained(
                "meta-llama/Llama-2-7b-chat-hf", token="LLaMA TOKEN"
            )
            print("Before model")
            ...
            self.model = LlamaForCausalLM.from_pretrained(
                "meta-llama/Llama-2-7b-chat-hf", config=config, token="LLaMA TOKEN"
            )
            print("Before DEVICE")
            self.model.to(DEVICE)
            print("Before eval")
    ...
    def _lexical_unit(self, sents):
        ...
        if self.sent_level_self_info:
            ...
            for sent in sents:
                ...
                noun_phrases, noun_phrases_info = self._calculate_lexical_unit(
                    tokens, self_info
                )
            ...
            return [
                ...
                LexicalUnits(
                    "phrase", text=all_noun_phrases, self_info=all_noun_phrases_info
                ),
    ...
    def self_info_mask(self, sents: List[str], self_info: List[float], mask_level):
        ...
        masked_context = (
            " ".join(sents_after_mask)
            if mask_level == "sent"
            else "".join(sents_after_mask)
        )
    ...
    def mask_a_sent(self, sent, level):
        if level == "phrase":
            ...
            if self.keep_leading_word:
                leading_few_words = (
                    " ".join(word_tokenize(sent)[: self.num_lead_words]) + " "
                )
            else:
                leading_few_words = ""
    ...
    def __call__(
        self, text: str, reduce_ratio: float = 0.35, reduce_level: str = "phrase"
    ) -> List[str]:
